...
def product_details(request, id_product):
    ...
    tienda = miTienda(person_id)
    return render(request, 'products.html', {'stripe_key': settings.STRIPE_PUBLISHABLE_KEY, 'product': product, 'types': productType, "context": context, "promotionProduct": not(promotionProduct), 'tienda': tienda})
